...
pred_interval = 3
data_dir = "pred_data_more_data_700_10/rf_%d" % pred_interval
save_dir = "dnn_plot_results_700_10/pred_%d" % pred_interval
...
def dnn_model(network, norm_train_set_x, train_set_y, norm_test_set_x, test_set_y):
    ...
    test_set_y_reverse = 1 - test_set_y
    test_set_y = np.transpose(np.vstack([test_set_y_reverse, test_set_y]))
...
for start_idx in start_idxes:
    ...
    for network in network_ary:
        ...
        history_data.append({
            ...
            'acc': np.copy(history.history['acc']),
            'val_acc': np.copy(history.history['val_acc'])
        })
    ...
    for epoch_num in [60, 20]:
        ...
        legend = []
        for idx, history in enumerate(history_ary):
            plt.plot(history.history['acc'][:epoch_num], color=color[idx], ls='--')
            plt.plot(history.history['val_acc'][:epoch_num], color=color[idx])
            legend.append("train: " + network_str_ary[idx])
            legend.append("val:   " + network_str_ary[idx])
        plt.title('model accuracy (predict interval=%d, start index=%d)' % (pred_interval, start_idx))
        plt.ylabel('loss')
        plt.xlabel('epoch')
        plt.legend(legend, loc='best')
        fig_path = "%s/acc_pred_%d_start_%d_epoch_%d.jpg" % (save_dir, pred_interval, start_idx, epoch_num)
        plt.savefig(fig_path)
